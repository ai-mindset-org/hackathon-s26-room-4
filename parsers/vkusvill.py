...
Роль уточнена заказчиком 29.08: розница — **полноценный источник закупки**,
а не только ориентир. Айгуль: если у оптовика не хватает объёма, недостающее
добирают в рознице. Значит ВкусВилл идёт в сравнение наравне с оптом, а не
отдельной справочной строкой.

⚠ Что при этом остаётся правдой: цена здесь розничная, то есть верхняя
граница. Если она оказывается ниже оптовой — это сигнал не «выгода», а
«проверь, тот ли это товар»: скорее всего разошлись фасовка или обработка. Оптовые цены в вебе не публикуются вообще — это
...
def snapshot(category: str = "мясо-птица") -> dict:
    ...
    try:
        ...
        return build_snapshot(f"ВкусВилл · поставщик (розница) · {category}", [],
                              status="unreachable")
    ...
    return build_snapshot(f"ВкусВилл · поставщик (розница) · {category}", items, status=status)
