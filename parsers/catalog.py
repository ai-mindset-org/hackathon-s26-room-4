...
# ⚠ Вес ТАРЫ — не основание делить цену. У оптовиков рыбы цена в прайсе стоит
# за килограмм, а рядом в названии — вес брикета или короба: «Тушка трески …
# Брикет 14кг Вес упаковки: 14». Разделив 460 ₽ на 14, инструмент выдал
# 33 ₽/кг и поставил треску первой строкой самого выгодного товара дня.
# Санитарный коридор это не поймал: 33 ₽ — правдоподобное число, и в этом
# вся ловушка. Отличить можно только по словам вокруг веса.
_PACKAGING = ("вес упаковки", "брикет", "короб", "ящик", "мешок",
              "фасовка", "вес нетто", "размерный ряд", "в упаковке")
...
def to_per_kg(title: str, price: float, source: str = "") -> tuple[float | None, str]:
    ...
    if any(mark in low for mark in _PACKAGING):
        return price, "цена за кг, число в названии — вес тары"
